"""Unit tests for the med-check timeline stub plumbing (Task 8) — no API calls.

Exercises _build_toolset's create_timeline_note capture-and-concatenate pattern
against TestModel (mirroring test_evals_smoke.py's approach: test the harness/
toolset wiring, never the real TARGET_MODEL task fn, which would spend money).
Also guards the fixture-completeness invariant documented in
evals/fixtures/med_check.py: every FIXTURES entry must carry every
fixture-driven tool key, or an unexpected tool call KeyErrors the task fn and
pydantic-evals silently drops the case.
"""
